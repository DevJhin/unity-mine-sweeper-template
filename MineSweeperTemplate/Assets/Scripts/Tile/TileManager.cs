    ...
    public class TileManager : MonoBehaviour
    {
        ...
        IEnumerator ProceduralSweep(Tile firstTile)
        {
            ...
            while (queue.Count != 0)
            {
                ...
                Debug.Log("Sweep" + queue.Count);
                yield return null;
            }
            Debug.Log("Sweep complete");
...
{
 // this.maxRow = playSettings.row;
 //  this.maxColumn = playSettings.column;

 this.mineNum = gameSettings.mineNumber;

 boardObject = Instantiate(boardPrefab) as GameObject;

 boardObject.transform.position = Vector3.zero;
 boardObject.transform.localScale = new Vector3(maxColumn, 0.25f, maxRow);

 boardOffset = new Vector3(-maxColumn + 0.5f, 1.25f, maxColumn);
